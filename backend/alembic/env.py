...
# database.py creates an async engine on import — ensure it gets an asyncpg URL
_db_url = os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./local_dev.db")
...
# Use sync psycopg2 URL for alembic (alembic doesn't support asyncpg directly)
def get_url():
